...
bool QnnConv2DRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  CHECK(IsScalarType(types[2], Int(32)));    // input_zero_point
  CHECK(IsScalarType(types[3], Int(32)));    // kernel_zero_point
  CHECK(IsScalarType(types[4], Float(32)));  // input_scale
  ...
  AssignType(types[5], Float(32), weight->shape[axis], reporter);  // kernel scale
...
Array<Array<Layout>> QnnConvInferCorrectLayout(const Attrs& attrs,
  ...
  auto layouts =
      Conv2DInferCorrectLayout<Conv2DAttrs>(attrs, new_in_layouts, old_in_layouts, old_in_types);
...
Expr Conv2DFallBack(const Expr& data, const Expr& weight, const Expr& input_zero_point,
  ...
  auto zp_data = Cast(input_zero_point, Int(16));
  auto zp_kernel = Cast(kernel_zero_point, Int(16));

  auto shifted_data = Cast(data, Int(16));
  auto zero_scalar = MakeConstantScalar(Int(32), 0);
...
Expr DepthwiseConv2DSecondTerm(const Expr& padded_data, const Expr& kernel_zero_point,
  ...
  if (kernel_h * kernel_w != 1) {
    auto scaled_hw_t2 =
        Multiply(casted_t2, MakeConstantScalar(Int(32), kernel_h * kernel_w));
  ...
  auto one_scalar = MakeConstantScalar(Int(32), 1);
...
Expr Conv2DSecondTerm(const Expr& padded_data, const Expr& kernel_zero_point,
  ...
  if (kernel_h * kernel_w != 1) {
    reduced_c_t2 =
        Multiply(reduced_c_t2, MakeConstantScalar(Int(32), kernel_h * kernel_w));
    reduced_t2 =
