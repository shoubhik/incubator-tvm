...
bool QuantizeRel(const Array<Type>& types,
  ...
  AssignType(types[1], Float(32), data->shape[axis], reporter);  // scale
  AssignType(types[2], Int(32), data->shape[axis], reporter);    // zero point
...
Expr QuantizeLower(const Expr& input_tensor, const Expr& output_scale,
  ...
  auto add_zero_point =
      Cast(Round(Add(scale_data, Cast(expanded_output_zero_point, Float(32)))),
           Int(32));
