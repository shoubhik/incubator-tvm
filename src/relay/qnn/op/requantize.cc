...
Expr RequantizeLower(const Expr& input_tensor, const Expr& input_scale,
  ...
  if (!IsEqualScalar(output_zero_point, zero_scalar)) {
    shifted_int32_t = Add(Cast(output_zero_point, Int(32)), scaled_int32_t);
  }
...
bool RequantizeRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  AssignType(types[1], Float(32), data->shape[axis], reporter);  // input_scale
  AssignType(types[2], Int(32), data->shape[axis], reporter);    // input_zero_pt
  // For now, requantize output tensor is limited to full tensor uniform quantization.
  CHECK(IsScalarType(types[3], Float(32)));  // output_scale
  CHECK(IsScalarType(types[4], Int(32)));    // output_zero_point

  const Array<tvm::Expr> oshape = data->shape;
  // assign output type
  auto out_dtype = requantize_attrs->out_dtype;
  CHECK(out_dtype == Int(8) ||
        out_dtype == UInt(8) ||
        out_dtype == Int(32))
