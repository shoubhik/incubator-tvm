...
inline Expr RequantizeOrUpcast(const Expr& expr,
                               ...
                               const Array<IndexExpr>& expr_shape,
                               const DataType& target_dtype = Int(32)) {
  auto result = expr;
...
static inline bool QnnBroadcastRel(const Array<Type>& types,
  ...
  CHECK(IsScalarType(types[2], Float(32)));  // lhs_scale
  CHECK(IsScalarType(types[3], Int(32)));    // lhs_zero_point
  CHECK(IsScalarType(types[4], Float(32)));  // rhs_scale
  CHECK(IsScalarType(types[5], Int(32)));    // rhs_zero_point
  CHECK(IsScalarType(types[6], Float(32)));  // output_scale
  CHECK(IsScalarType(types[7], Int(32)));    // output_zero_point
